...
func NewEncrypter() *Encrypter {
	key := os.Getenv("KEY")
	if key == "" {
		panic("Не реоедан параметр KEY в переменные окружения")
	}
	return &Encrypter{
		Key: key,
	}
...
func (enc *Encrypter) Decrypt(encrypedStr []byte) []byte {
	block, err := aes.NewCipher([]byte(enc.Key))
	if err != nil {
		panic(err.Error())
	}
	...
	nonce, cipherText := encrypedStr[:nonceSize], encrypedStr[nonceSize:]
